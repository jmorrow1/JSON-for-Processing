...
// ISSUE: if a subclass has a field of the same name as its superclass, only the
// superclass field gets saved.
// ISSUE: if a class extends a class from an external library, it can't be a
// JSONer so there needs to be a way for the user
// to incorporate such classes into the saving and loading process. Furthermore,
// even if the user defines their own
// class hierarchy but only wants some of the subclasses to extend JSONer, we'll
// have a problem.
...
public abstract class JSONer implements JSONable {
    ...
    private static JSONObject toJSON(Object obj) {
        ...
        // write the values of the this class's declared fields to the json
        // object.
